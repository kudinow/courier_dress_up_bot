...
ADMIN_IDS = {91892537, 52227231, 80655741}
...
def _get_user_data(user_id: int) -> dict:
    ...
    if user_key not in data:
        return {"generations": 0, "last_photo_url": None}

    # Поддержка старого формата (просто число)
    user_data = data[user_key]
    if isinstance(user_data, int):
        return {"generations": user_data, "last_photo_url": None}
...
def save_last_photo_url(
    user_id: int, photo_url: str, gender: str = "male"
) -> None:
    """Сохраняет последнюю фотографию и пол пользователя"""
    user_data = _get_user_data(user_id)
    user_data["last_photo_url"] = photo_url
    user_data["gender"] = gender
    _set_user_data(user_id, user_data)
    logger.info(f"Saved last photo for user {user_id}, gender={gender}")


def get_last_photo(user_id: int) -> Optional[str]:
    """Возвращает URL последней фотографии пользователя"""
    user_data = _get_user_data(user_id)
    return user_data.get("last_photo_url")


def get_last_gender(user_id: int) -> str:
    """Возвращает пол, выбранный при последней генерации"""
    user_data = _get_user_data(user_id)
    return user_data.get("gender", "male")
