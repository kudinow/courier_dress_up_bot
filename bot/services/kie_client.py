...
class KieClient:
    ...
    async def add_logo_to_photo(
        self,
        image_url: str,
        logo_url: str,
    ) -> str:
        """
        Второй этап: вписывает логотип на одежду курьера через kie.ai.

        Args:
            image_url: URL результата первой генерации
            logo_url: URL логотипа (PNG)

        Returns:
            str: URL готового изображения с логотипом
        """
        prompt = (
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, headers=self.headers) as resp:
                if resp.status != 200:
                    text = await resp.text()
                    logger.error(f"Failed to create logo task: {resp.status} - {text}")
                    raise KieClientError(f"Failed to create logo task: {resp.status}")
                ...
                task_id = data.get("data", {}).get("taskId")
                if not task_id:
                    raise KieClientError("No taskId in logo task response")

                logger.info(f"Created logo task: {task_id}")
