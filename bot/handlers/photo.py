...
from bot.services.cta_scheduler import schedule_cta
...
async def handle_photo(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    try:
        ...
        save_last_photo_url(user_id, file_url, gender)

        # Формируем caption с информацией об оставшихся генерациях
        remaining_after = get_remaining_generations(user_id)
        if remaining_after == -1:
            caption = "Вот твоя фотография в образе курьера 🚀"
        elif remaining_after > 0:
            caption = (
                f"Вот твоя фотография в образе курьера 🚀\n\n"
                f"📊 Осталось бесплатных генераций: {remaining_after}"
            )
        else:
            caption = (
                "Вот твоя фотография в образе курьера 🚀\n\n"
        ...
        schedule_cta(user_id, message.chat.id, bot)
