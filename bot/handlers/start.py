...
from bot.services.cta_scheduler import cancel_cta, schedule_cta
...
async def restart_generation(
    ...
    cancel_cta(callback.from_user.id)
    await state.clear()
...
async def regenerate_photo(callback: CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    # Получаем последнюю фотографию и пол
    photo_url = get_last_photo(user_id)
    gender = get_last_gender(user_id)
    ...
    from bot.services.logo import LOGO_URL
    ...
    try:
        ...
        # Скачиваем результат
        result_image = await kie_client.download_image(final_url)
        ...
        # Планируем отложенное CTA-сообщение
        schedule_cta(user_id, callback.message.chat.id, bot)
